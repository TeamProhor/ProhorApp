...
export default function Shell({ children, dict, lang }: ShellProps) {
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-muted text-foreground font-sans overflow-hidden relative">
      {/* Mobile Top Navbar */}
      <div className="lg:hidden absolute top-0 left-0 right-0 z-20 flex flex-col gap-[8px] p-[16px]">
        <div className="flex flex-row items-center justify-between bg-muted border-[0.5px] border-border rounded-[24px] p-[16px] shadow-sm">
          <Button
            variant="ghost"
            size="icon"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="flex flex-col justify-center invisible p-0 h-auto"
          >
            <List size={28} className="size-7" />
          </Button>
        </div>
      </div>
